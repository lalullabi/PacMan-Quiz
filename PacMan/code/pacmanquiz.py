...
# data kuis dengan pertanyaan, jawaban yang benar, dan opsi
quiz_data = [
    {"question": "Bahasa resmi Brasil adalah?",
     "answers": ["c. Portugis"], 
     "options": ["a. Spanyol", "b. Inggris", "c. Portugis", "d. Prancis"]},
    {"question": "Sungai terpanjang di dunia?",
     "answers": ["a. Sungai Nil"],
     "options": ["a. Sungai Nil", "b. Sungai Amazon", "c. Sungai Mississipi", "d. Sungai Yangtze"]},
    {"question": "Siapakah presiden pertama Republik Indonesia?",
     "answers": ["b. Sukarno"],
     "options": ["a. Soeharto", "b. Sukarno", "c. Joko Widodo", "d. Megawati Soekarnoputri"]},
    {"question": "Planet terdekat dengan Matahari adalah?",
     "answers": ["c. Merkurius"],
     "options": ["a. Venus", "b. Mars", "c. Merkurius", "d. Bumi"]},
    {"question": "Laut terdalam di dunia adalah?",
     "answers": ["c. Palung Mariana"],
     "options": ["a. Laut Karibia", "b. Samudra Pasifik", "c. Palung Mariana", "d. Laut Mediterania"]}
    ...
    while waiting_for_answer:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # Periksa opsi mana yang diklik
                option_y = 250
                for i, option in enumerate(options):
                    option_rect = pygame.Rect(100, option_y, option_text.get_width(), option_text.get_height())
                    if option_rect.collidepoint(mouse_x, mouse_y):
                        selected_answer = option
                        # Hapus area jawaban sebelumnya
                        pygame.draw.rect(screen, '#151515', option_rect)  # Menghapus jawaban sebelumnya
                        if selected_answer in correct_answers:
                            ...
                            print("Jawaban salah!")
                            option_color = 'red'
                            return False  # Game over atau kurangi nyawa di sini

                        # Gambar ulang opsi dengan warna yang benar
                        option_text = font.render(selected_answer, True, option_color)
                        screen.blit(option_text, (100, option_y))
                        waiting_for_answer = False
                        break  # Keluar dari loop for jika jawaban sudah dipilih
                    option_y += 30

        pygame.display.update()  # Update seluruh layar setelah memproses event
        pygame.time.delay(500)  # Delay untuk memberi waktu melihat warna
    ...
    while run:
        ...
        if not powerup:
            if (player_circle.colliderect(
                ...
                # quiz
                if quiz(): # Panggil fungsi quiz dan periksa hasilnya
                # Jika jawaban benar, lanjutkan permainan
                # (misalnya, reset posisi hantu, dll.)
                    pass  
                elif lives > 0: # Jika jawaban salah, kurangi nyawa
                        lives -= 1
                    # ... (kode untuk reset posisi, dll.) ...
                else:
                    game_over = True
                    moving = False
                    startup_counter = 0
